...
def main(n, t_End, d_t, nb_l):
	""" SPH simulation """
	
	# Simulation parameters
	N         = n    # Number of particles
	t         = 0      # current time of the simulation
	tEnd      = t_End      # time at which simulation ends 
	dt        = d_t   # timestep 0.1 avec 800 est bien aussi
	...
	for i in range(Nt):
        ...
        ########################################################################
        ######### Number of particles rendred in Low resolution ################
		nbL = nb_l  # 2 means only 1 of 2 particles, out of the window, are rendred
        ########################################################################
        ########################################################################
		reducesize = 0
		for j in range(N) :
...
if __name__== "__main__":
	...
	dt        = 0.05   # timestep 
	nbl       = 2     # ratio of outside to inside area of interest particles

	
	with open("log_2scale.txt", 'a') as f:
		start = time.perf_counter()
		main(N, tEnd, dt, nbl)
		end = time.perf_counter()
		f.write(f'{N=}, {dt=}, {nbl=}, {end-start}\n')
